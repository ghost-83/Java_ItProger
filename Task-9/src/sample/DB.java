...
public class DB {
    ...
    public ResultSet getArticles() throws SQLException, ClassNotFoundException {
        String sql = "SELECT `id`, `title`, `intro`, `data` FROM `articles`;";
        Statement statement = getDBConnection().createStatement();
        return statement.executeQuery(sql);
    }

    public ResultSet getArticle(int id) throws SQLException, ClassNotFoundException {
        String sql = "SELECT `title`, `text`, `data` FROM `articles` WHERE `id` = '" + id + "' LIMIT 1;";
        Statement statement = getDBConnection().createStatement();
        return statement.executeQuery(sql);
    }

    public void addNews(String title, String intro, String text) throws SQLException, ClassNotFoundException {
        String sql = "INSERT INTO `articles` (title, intro, text, data) VALUES (?,?,?,?);";
